...
//读取task chan并分发到worker goroutinue处理，总的数量是workers
...
//工作goroutinue处理具体工作，并将处理结果发送到结果chan
...
//通过done channel同步等待所有工作goroutinue的结束，然后关闭结果chan
...
func main() {
	...
	//初始化task的goroutinue,计算100个自然数之和
	go InitTask(taskchan,resultchan,100)
	//分发任务到NUMBER个goroutinue池
	DistributeTask(taskchan,workers,done)
	//获取各个goroutinue处理完任务的通知，并关闭结果通道
